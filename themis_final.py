...
def calculate_advanced_metrics(df, spy_df):
    try:
        ...
        dna = round(rebel * 0.6 + 40, 1) # 簡化版真實基因
...
def calculate_filtered_energy(df):
    try:
        ...
        real_move = abs(df['Close'].iloc[-1] - df['Close'].iloc[-5])
        if real_move < (atr * 1.5): return 50.0
...
try:
    asset = yf.Ticker(ticker); info = asset.info; df = asset.history(period="2y")
    ...
    if not df.empty:
        ...
        fig.add_trace(go.Bar(x=counts, y=bins[:-1], orientation='h', marker_color='rgba(0, 255, 204, 0.4)', name="蟹貨成交"), row=1, col=2)
        ...
        m1.write(f"**PS (營收比):** {info.get('priceToSalesTrailing12Months', 'N/A')}")
        m2.write(f"**EV/EBITDA:** {info.get('enterpriseToEbitda', 'N/A')}")
        m2.write(f"**股息率:** {info.get('dividendYield', 0)*100:.2f}%")
        m3.write(f"**Beta:** {info.get('beta', 'N/A')}")
        m3.write(f"**市值:** {info.get('marketCap', 0)/1e9:.1f}B")
    ...
    st.error(f"數據對準中... 請確保代號輸入正確。 (Error: {e})")
