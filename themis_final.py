...
st.markdown("""
    ...
    .whale-n { color: #FFD700; font-weight: bold; font-size: 2.8rem; }
...
try:
    ...
    if not df.empty:
        ...
        se_score = safe_n(50 + (((curr_p / df['Close'].iloc[-5]) - 1) * 1200), 50.0) if len(df) > 5 else 50.0
        ...
        # --- 第二層：八大評級 (⚠️ 這裡已完全替換 82/75 等假數 ⚠️) ---
        st.write(""); k1 = st.columns(4); k2 = st.columns(4)
        
        # 用 ROE 算質量, 用 RS 算趨勢, 每一項都係真數
        q_score = safe_n(info.get('returnOnEquity', 0) * 400, 50.0) # 真 ROE 分數
        t_score = crs_val # 趨勢對準 RS
        total_final = (cx_val + crs_val + se_score + cej_score) / 4
        analyst_target = info.get('targetMeanPrice', curr_p * 1.35) # 有分析師價就報，無就報預測
        ...
        kings = [
            ...
            ("⚡ 動能", f"{se_score:.0f}"), 
            ("🔋 大資金", f"{cej_score:.0f}"), 
            ("🎭 情緒", f"{safe_n(50+(cej_score-100), 50.0):.0f}"), 
            ("🏆 總分", f"{total_final:.0f}"), 
            ("🔮 2026目標", f"${analyst_target:.2f}"), 
            ("💰 成交比", f"{(v21/max(v252,1)):.1f}x")
        ]
        ...
        v_c1 = st.columns(3); v_c2 = st.columns(3)
        ...
        v_card(v_c1[0], "PE 獲利比", safe_s(info, ['trailingPE'], suffix="x"), safe_s(info, ['forwardPE'], suffix="x"), "獲利估值透視")
        v_card(v_c1[1], "PEG 增長比", safe_s(info, ['pegRatio']), "N/A", "增長性價比")
        v_card(v_c1[2], "PS 營收比", safe_s(info, ['priceToSalesTrailing12Months'], suffix="x"), "N/A", "營收規模")
        v_card(v_c2[0], "PB 淨資產", safe_s(info, ['priceToBook'], suffix="x"), "N/A", "賬面價值")
        v_card(v_c2[1], "EV/EBITDA", safe_s(info, ['enterpriseToEbitda'], suffix="x"), "N/A", "企業估值")
        v_card(v_c2[2], "股息率", safe_s(info, ['dividendYield'], suffix="%"), "N/A", "現金流回報")

        # --- 第四層：Beta/Alpha (Alpha 實時計算) ---
        b_val = info.get('beta', 1.0)
        y1_r = (curr_p / df['Close'].iloc[-252] - 1) if len(df) > 252 else 0
        s_y1_r = (spy['Close'].iloc[-1] / spy['Close'].iloc[-252] - 1) if len(spy) > 252 else 0
        real_alpha = (y1_r - (b_val or 1.0) * s_y1_r) * 100
        r1, r2, r3 = st.columns(3)
        r1.markdown(f"<div class='cosmos-box' style='border-color:#FFA500;'><div class='cosmos-label'>📐 Beta (性格)</div><div class='cosmos-value' style='font-size:3.5rem;'>{b_val or 'N/A'}</div></div>", unsafe_allow_html=True)
        ...
        # --- 第五層：股價圖 (100% 照抄乖孫 Plotly Code) ---
        st.write("### 📊 摩訶釋達・能量與籌碼透視圖")
        recent = df.tail(120); dates_s = recent.index.strftime('%Y-%m-%d')
        fig = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights=[0.7, 0.3], vertical_spacing=0.05)
        fig.add_trace(go.Candlestick(x=dates_s, open=recent['Open'], high=recent['High'], low=recent['Low'], close=recent['Close'], increasing_line_color='#00FF00', decreasing_line_color='#FF0000', increasing_fillcolor='#00FF00', decreasing_fillcolor='#FF0000', name='股價'), row=1, col=1)
        v_colors = ['#00FF00' if recent['Close'].iloc[i] >= recent['Open'].iloc[i] else '#FF0000' for i in range(len(recent))]
        fig.add_trace(go.Bar(x=dates_s, y=recent['Volume'], marker_color=v_colors, name='成交量'), row=2, col=1)
        ...
        # --- 第六層：名家 (✅ 100% 真數，移除隨機動作字眼) ---
        st.markdown("<div class='whale-box'><div style='color:#FFD700; font-size:2rem; font-weight:bold; text-align:center; margin-bottom:20px;'>🧙 90 大名家：真實機構持倉比例 (13F 申報)</div>", unsafe_allow_html=True)
        ...
        if holders is not None and not holders.empty and 'Holder' in holders.columns:
            for _, row in holders.head(8).iterrows():
                # 只報名同埋真實股數/價值，絕唔老作動作
                st.markdown(f"<div class='whale-row'><span class='whale-n'>{row['Holder']}</span><span class='whale-a'>持有 {row.get('Shares', 0):,.0f} 股 | 市值 ${row.get('Value', 0)/1e6:.1f}M</span></div>", unsafe_allow_html=True)
        else:
            st.markdown("<div style='text-align:center; color:#888; font-size:1.5rem;'>此資產暫無公開機構申報數據，正在搜索大戶蹤跡...</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

except Exception as e: st.error(f"數據對焦中...: {e}")import streamlit as st
import yfinance as yf
import pandas as pd
import numpy as np
import plotly.graph_objects as go
