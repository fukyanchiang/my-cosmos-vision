...
st.markdown(""" 
    ...
    .cosmos-box { background-color: #000 !important; border: 4px solid 
#00FFCC; border-radius: 20px; padding: 25px; text-align: center; 
box-shadow: 0 0 20px #00FFCC44; } 
    ...
    .val-box { background-color: #000 !important; border: 2px solid 
#FFD700; border-radius: 12px; padding: 20px; text-align: center; 
min-height: 200px; } 

    .val-label { color: #FFFFFF !important; font-size: 1.6rem; 
font-weight: bold; border-bottom: 2px solid #444; padding-bottom: 8px; 
margin-bottom: 12px; } 
    .val-text { font-size: 1.3rem; color: #ccc; margin: 8px 0; } 
    .val-focus { color: #FFD700; font-weight: bold; font-size: 1.8rem; } 

    .red-bar { background-color: #FF4B4B; color: #fff; padding: 20px; 
border-radius: 10px; text-align: center; font-weight: 900; font-size: 
2.5rem; margin: 30px 0; border: 3px solid #fff; } 

    .val-box-purple { border: 3px solid #BC13FE; border-radius: 15px; 
padding: 30px; background-color: #000; box-shadow: 0 0 25px #BC13FE66; 
margin: 25px 0; } 
...
try: 
    ...
    if not df.empty: 
        ...
        crs_val = safe_n(50 + ((curr_p / df['Close'].iloc[-63]) - 
(spy['Close'].iloc[-1] / spy['Close'].iloc[-63])) * 100, 50.0) if \
len(df) > 63 else 50.0 
        ...
        c1.markdown(f"""<div class='cosmos-box'><div 
class='cosmos-label'>COSMOS-X (天體動能)</div><div 
class='cosmos-value'>{cx_val:.1f}</div></div>""", 
unsafe_allow_html=True) 

        c2.markdown(f"""<div class='cosmos-box' 
style='border-color:#FFD700;'><div 
class='cosmos-label'>COSMOS-RS (星系強弱)</div><div 
class='cosmos-value'>{crs_val:.1f}</div></div>""", 
unsafe_allow_html=True) 
        with c3: 

            st.markdown("""<div class='cosmos-box' 
style='border-color:#00FFFF; padding: 20px;'>""", 
unsafe_allow_html=True) 
            def draw_triad_bar(val, title, color): 
                ...
                for g in range(7): 
                    ...
                    for i in range(3): 
                        ...
                        html += f"<div class='ej-seg' 
style='background-color:{c_code if idx < lit else '#222'}; 
opacity:{op}; {sh}'></div>" 
                    html += "</div>" 
                return html + "</div>" 
        ...
        with d_c1: 
            ...
            <div class='cosmos-box' style='border-color:#FF4B4B; 
...
"".join([f'<div class="energy-seg-8d" \
style="background-color:{colors_8d[i%8]}; opacity:{"1" if j<=sc else \
"0.1"};"></div>' for j in range(1,11)]) + '</div>' 

                st.markdown(f"""<div style='display:flex; \
justify-content:space-between; \
font-weight:bold;'><span>{label}</span><span>{sc}/10</span></div>{grid}""",
 unsafe_allow_html=True) 
        ...
        for i in range(4): 
            ...
            k1[i].markdown(f"""<div class='cosmos-box' \
style='padding:15px; border-width:2px;'><div style='color:#ccc; \
font-size:1.2rem;'>{kings[i][0]}</div><div \
style='color:#FFD700; font-size:2.5rem; \
font-weight:bold;'>{kings[i][1]}</div></div>""", \
unsafe_allow_html=True) 

            k2[i].markdown(f"""<div class='cosmos-box' \
style='padding:15px; border-width:2px; border-color:#FFD700;'><div\
 style='color:#ccc; \
font-size:1.2rem;'>{kings[i+4][0]}</div><div \
style='color:#FFD700; font-size:2.5rem; \
font-weight:bold;'>{kings[i+4][1]}</div></div>""", \
unsafe_allow_html=True) 
        ...
        def v_card(col, title, t_val, f_val, desc): 
            ...
            col.markdown(f"""<div class='val-box'><div \
class='val-label'>{title}</div><div class='val-text'>TTM:\
 <span class='val-focus'>{t_val}</span></div><div \
class='val-text'>12M預期: <span \
class='val-focus'>{f_val}</span></div><div \
style='color:#FFA500; font-size:0.9rem; \
margin-top:10px;'>{desc}</div></div>""", \
unsafe_allow_html=True) 
        ...
        if not is_etf: 
            ...
            <div style='border: 4px solid {val_color}; 
border-radius: 15px; padding: 30px; background-color: #000; box-shadow: 0
 0 30px {val_color}66; margin: 25px 0;'> 
                <div style='display:flex; justify-content:space-between; align-items:center;'> 
                    <div> 

                        <span style='font-size:2.2rem; 
font-weight:900;'>COSMOS-VAL 解碼：<span 
style='color:{val_color};'>{val_title}</span>{warning_html}</span><br>
 

                        <span style='font-size:1.1rem; 
opacity:0.8;'>（針對 TTM PE {ttm_pe:.2f}x 獨立戰術評分）</span><br>
 

                        <span style='font-size:1.2rem; 
color:#FFD700; font-weight:bold; margin-top:5px; 
display:inline-block;'>[ 註明：共分 4 級，現在這公司基於真實財報屬 {t_lv} ({t_desc}) 
]</span> 
                    </div> 
...
border-radius:10px; margin-top:20px; border:1px solid #333;'> 

                    <b style='color:white; 
font-size:1.3rem;'>真實財報決策指令：</b> <span 
style='color:{val_color}; font-size:1.3rem;'>{act_desc}</span> 
                </div> 
            </div>""", unsafe_allow_html=True) 
        ...
        r1.markdown(f"""<div class='cosmos-box' \
style='border-color:#FFA500;'><div class='cosmos-label'>📐 Beta\
 (性格)</div><div class='cosmos-value' \
style='font-size:3.5rem;'>{b_val:.2f}</div></div>""", \
unsafe_allow_html=True) 

        r2.markdown(f"""<div class='cosmos-box' \
style='border-color:#FFA500;'><div class='cosmos-label'>🔱 \
Alpha (超額)</div><div class='cosmos-value' \
style='font-size:3.5rem;'>{real_alpha:.1f}%</div></div>""",\
 unsafe_allow_html=True) 

        r3.markdown(f"""<div class='cosmos-box' \
style='border-color:#FFA500;'><div class='cosmos-label'>🌊 波動率 \
(情緒)</div><div class='cosmos-value' \
style='font-size:3.5rem;'>{(v_ann*100):.1f}%</div></div>""",\
 unsafe_allow_html=True) 
        ...
        fig.add_trace(go.Candlestick(x=dates, open=recent['Open'], \
high=recent['High'], low=recent['Low'], close=recent['Close'], \
name='股價'), row=1, col=1) 

        fig.add_trace(go.Bar(x=dates, y=recent['Volume'], \
marker_color=['#00FF00' if recent['Close'].iloc[i] >= \
recent['Open'].iloc[i] else '#FF0000' for i in range(len(recent))], \
name='成交量'), row=2, col=1) 
        counts, bins = np.histogram(recent['Close'], bins=20, weights=recent['Volume']) 

        fig.add_trace(go.Bar(y=(bins[:-1] + bins[1:]) / 2, x=counts, \
orientation='h', marker_color='rgba(0, 255, 204, 0.4)', name='蟹貨', \
xaxis='x3', yaxis='y1')) 

        fig.update_layout(template="plotly_dark", \
paper_bgcolor='#0e1117', plot_bgcolor='#0e1117', height=750, \
showlegend=False, xaxis_rangeslider_visible=False, \
xaxis=dict(type='category', showgrid=False), yaxis=dict(showgrid=True, \
gridcolor='#333'), yaxis2=dict(showgrid=False), \
xaxis3=dict(overlaying='x', side='top', range=[0, max(counts)*6], \
showgrid=False, showticklabels=False)) 
        st.plotly_chart(fig, use_container_width=True) 

        # 名家清單 

        st.markdown("""<div class='whale-box'><div \
style='color:#FFD700; font-size:2.2rem; font-weight:bold; \
text-align:center; margin-bottom:20px;'>🧙 90 大名家：真實申報持倉 \
(自動計算佔比)</div>""", unsafe_allow_html=True) 
        ...
        if holders is not None and not holders.empty and 'Holder' in holders.columns: 
            for _, row in holders.head(8).iterrows(): 
                ...
                st.markdown(f"""<div class='whale-row'><span \
class='whale-n'>{row['Holder']}</span><span \
class='whale-a'>持有 {shares:,.0f} 股 | 佔比 {calc_pct:.2%} | 市值 \
${val_m:.1f}M</span></div>""", unsafe_allow_html=True) 
        else: 

            st.markdown("""<div style='text-align:center; \
color:#888; padding:20px;'>此資產暫無公開機構申報數據</div>""", \
unsafe_allow_html=True) 
        st.markdown("""</div>""", unsafe_allow_html=True) 

except Exception as e: st.error(f"數據診斷中: {e}")import streamlit as st
import yfinance as yf
import pandas as pd
import numpy as np
import plotly.graph_objects as go
