...
# 1. 基礎設置
st.set_page_config(page_title="COSMOS 戰略指揮部 V44.1", layout="wide")
            ...
            try:
                ...
                if len(df) > 63:
                    ...
                    if se_s > 85 and cej_s > 110 and crs_val > 52:
                        breakout_found = True
                        st.markdown(f"<div class='scan-card scan-card-fire'><h2 style='color:#FFD700; margin:0;'>🎯 {t} | 能量共振！</h2><p style='margin:5px 0; color:#ddd;'>🧬 DNA: <b>{dna_v}</b> | ⚡ SE: <b style='color:#FF4B4B;'>{se_s:.1f}</b> | 🔋 EJ: <b style='color:#00FFCC;'>{cej_s:.1f}</b> | 📈 RS: <b>{crs_val:.1f}</b></p></div>", unsafe_allow_html=True)
                    else:
                        st.markdown(f"<div class='scan-card'><h4 style='color:#ccc; margin:0;'>💤 {t} | 尚未到達爆發點</h4><span style='font-size:0.9rem; color:#888;'>動能 SE: {se_s:.1f} | 資金 EJ: {cej_s:.1f}</span></div>", unsafe_allow_html=True)
            except: pass
            
        if not breakout_found: st.warning("掃描完畢：此版塊目前無起步訊號。")

# ==========================================
# 模式 C：個股深度透視 (100% 完整無缺)
# ==========================================
else:
    ...
    try:
        ...
        if not df.empty:
            ...
            with d_c1:
                st.markdown(f"<div class='cosmos-box' style='border-color:#FF4B4B; height:380px; display:flex; flex-direction:column; justify-content:center;'><div style='color:#FF4B4B; font-weight:900; font-size:1.8rem;'>🧬 COSMOS-DNA</div><div style='font-size:0.9rem; opacity:0.7; margin:5px 0;'>{dna_title}</div><div style='font-size:6rem; font-weight:900;'>{dna_v}</div><div style='color:#FFD700; margin-top:10px;'>[ 現屬 {d_lv} ]<br><span style='font-size:1.6rem; color:#FFF;'>{d_desc}</span></div></div>", unsafe_allow_html=True)
            with d_c2:
