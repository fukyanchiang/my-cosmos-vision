...
try:
    ...
    if not df.empty:
        ...
        # 🌌 COSMOS-RS (修復 SyntaxError 位置)
        if len(df) > 63 and len(spy) > 63:
            rel_ret = (curr_p / df['Close'].iloc[-63]) - (spy['Close'].iloc[-1] / spy['Close'].iloc[-63])
            crs_val = safe_n(50 + (rel_ret * 100), 50.0)
        else: crs_val = 50.0
        ...
        dna_s = round(safe_n(real_roe * 300 + 20, 23.6), 1)
        with d_c1:
            st.markdown(f"<div class='cosmos-box' style='border-color:#FF4B4B; height:380px; display:flex; flex-direction:column; justify-content:center;'><div style='color:#FF4B4B; font-weight:900;'>🧬 COSMOS-DNA</div><div style='font-size:0.9rem; opacity:0.7;'>投行級股王基因 (100分滿分)</div><div style='font-size:6rem; font-weight:900;'>{dna_s}</div></div>", unsafe_allow_html=True)
        with d_c2:
            st.markdown(f"**{ticker} ・ 8D 投行精確透視 BAR**")
            m8 = {
                "🩸 血液純度": int(safe_n(info.get('operatingMargins', 0)*30+3, 5)),
                "🛡️ 免疫系統": int(safe_n(real_roe*30+3, 5)),
                "🏗️ 心跳頻率": int(safe_n(info.get('revenueGrowth', 0)*20+4, 5)),
                "🧬 大腦潛力": int(safe_n(info.get('profitMargins', 0)*30+3, 5)),
                "🧱 骨架重量": int(max(1, 10 - safe_n(info.get('priceToBook', 5), 5))),
                "⚡ 物理底盤": 8 if safe_n(info.get('debtToEquity', 150), 150) < 80 else 4,
                "💰 資本配置": int(safe_n(info.get('dividendYield', 0)*150+2, 5)),
                "📈 經營拐點": int(safe_n(info.get('earningsGrowth', 0)*20+4, 5))
            }
        ...
        kings = [("📁 質量", f"{dna_s:.0f}"), ("📈 趨勢", f"{crs_val:.0f}"), ("⚡ 動能", f"{se_s:.0f}"), ("🔋 大資金", f"{cej_s:.0f}"), 
                 ("🎭 情緒", f"{safe_n(crs_val*0.9, 50):.0f}"), ("🏆 總分", f"{(cx_val+crs_val+se_s)/3:.0f}"), ("🔮 2026目標", f"${info.get('targetMeanPrice', curr_p*1.35):.2f}"), ("💰 成交比", f"{(v21/max(v252,1)):.1f}x")]
        for i in range(4):
        ...
        r2.markdown(f"<div class='cosmos-box' style='border-color:#FFA500;'><div class='cosmos-label'>🔱 Alpha (超額)</div><div class='cosmos-value' style='font-size:3.5rem;'>{r_alpha:.1f}%</div></div>", unsafe_allow_html=True)
        ...
        # 名家 (中文巨字+真數)
        st.markdown("<div class='whale-box'><div style='color:#FFD700; font-size:2.2rem; font-weight:bold; text-align:center; margin-bottom:20px;'>🧙 90 大名家：真實機構持倉名單</div>", unsafe_allow_html=True)
        ...
        if holders is not None and not holders.empty and 'Holder' in holders.columns:
            ...
            st.markdown("<div style='text-align:center; color:#888;'>暫無機構數據</div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
