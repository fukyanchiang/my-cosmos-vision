...
st.markdown("""
    ...
    /* 🚀 白底框框（Selectbox、Button、TextInput）裡面的字強制變黑色！ */
...
# =========================================================================
# 🛡️ 2. 強力緩存引擎 (Caching) 全部鎖入 1 小時！任拉 Slider 唔會報錯
# =========================================================================
@st.cache_data(ttl=3600)
def fetch_history(ticker_sym, period="1y"):
    try:
        time.sleep(0.3)
        return yf.Ticker(ticker_sym).history(period=period).dropna(subset=['Close', 'Volume'])
...
US_STOCK_MAP = {
    "1. 半導體設備與設計": "NVDA TSM AVGO ASML AMD QCOM TXN MU INTC AMAT LRCX KLAC ADI NXPI MRVL MCHP SWKS MPWR ON LSCC TER QRVO SLAB WOLF SYNA RMBS ALGM SITM ACLS CRUS DRAM".split(),
...
# ----------------- 🔘 側邊欄控制 -----------------
st.sidebar.markdown("## 🛰️ 戰術控制台 (V176.0)")
...
# =========================================================================
# 🚀 模式 A：個股深度透視 (🌟 修改點 3: 加番 「🔍 確認透視」 掣)
# =========================================================================
if app_mode == "🚀 個股深度透視":
    st.markdown("<h1 class='main-title'>🚀 個股深度透視</h1>", unsafe_allow_html=True)
    c_input, c_btn = st.columns([2, 1])
    with c_input: ticker = st.text_input("🚀 輸入資產代號", "6869.HK").upper()
    with c_btn: st.write("##"); run_a = st.button("🔍 確認透視")
    ...
    if run_a:
        with st.spinner(f"⏳ 系統正在切換引擎，重新為您下載海量數據及繪製摩訶圖... 請稍候 ☕🚀"):
            try:
                ...
                b_df_plot = fetch_history(b_sym_plot, period="2y")
                ...
                if not df.empty and not spy.empty:
                    ...
                    # 👴 🌟 修改點 5 & 6: 上市不足提醒 + 同上市第一日比避開 N/A + Cap OBV
                    ...
                    if not holders.empty and 'Holder' in holders.columns:
...
# =========================================================================
    ...
    with st.spinner(f"⏳ 大宗師正在計算市寬數據... 請稍候 ☕🚀"):
        try:
            idx_df = fetch_history(ticker_sym, period="2y")
            if not idx_df.empty:
...
# =========================================================================
# 🔍 模式 C/F/G：起步尋龍雷達 (個股/ETF) (🌟 修改點 1: 用番上圖邏輯)
# =========================================================================
    ...
    if st.button("📡 發射撒網尋龍電波！"):
        bench_data = fetch_history(bench_sym, period="2y")
        ...
        with st.spinner("⏳ 慢速防封鎖引擎過濾中，請稍候..."):
            for idx, t in enumerate(tickers_to_scan):
                ...
                try:
                    d_full = fetch_history(t, period="1y")
                    if len(d_full) > 100:
                        d = d_full.tail(63)
...
# =========================================================================
    ...
    with st.spinner('拔河排名計算中，慢速防封鎖引擎已啟動...'):
        try:
            bench_df = fetch_history(bench_sym, period="60d")['Close'].dropna(); results = []
            for name, tickers in target_map.items():
                for idx, t in enumerate(tickers):
                    try:
                        d = fetch_history(t, period="60d")['Close'].dropna()
                        if len(d) >= 20:
...
# =========================================================================
    ...
    with c_sec: s_choice = st.selectbox("3. 選擇掃描範圍", ["🌐 啟 মাতৃ星系大規模搜索"] + list(target_dict.keys()))
    ...
    if st.button("📡 [神掣] 發射！執行核心 RS 海選與大戶偵測"):
        ...
        with st.spinner("⏳ 慢速防封鎖引擎已啟動，正在對比大盤 RS 同尋找大戶足跡..."):
            try:
                bench_df = fetch_history(bench_sym, period="1y")['Close'].dropna()
                yearly_returns = {}
                valid_dfs = {}
                for idx, t in enumerate(tickers_to_scan):
                    pb.progress((idx + 1) / len(tickers_to_scan))
                    if idx > 0 and idx % 10 == 0: time.sleep(1.0)
                    try:
                        df_t = fetch_history(t, period="1y")
                        if len(df_t) > 150:
    ...
    if st.session_state.vcp_scanned_stocks:
        ...
        if selected_stock:
            ...
            with st.spinner("正在繪製 K線、重貨區 HVN 及 RS 領先線..."):
                try:
                    df = fetch_history(selected_stock, period="6mo")
                    b_df = fetch_history(bench_sym, period="6mo")['Close']
    ...
    if st.session_state.e_scanned_stocks:
        ...
        if sel:
            ...
            with st.spinner("正在為您繪製專屬海龜回測戰術圖表..."):
                try:
                    df = fetch_history(sel, period="6mo")
                    df['MA50'] = df['Close'].rolling(50).mean()
                    df['EMA10'] = df['Close'].ewm(span=10, adjust=False).mean()
