...
# 1. 基礎設置
st.set_page_config(page_title="COSMOS 戰略指揮部 V44.3", layout="wide")
...
US_SECTOR_MAP = {
    ...
    "房地產 (XLRE)": "XLRE", "銀行 (KRE)": "KRE", "建築 (ITB)": "ITB", "油服 (OIH)": "OIH", "生技 (IBB)": "IBB"
}
...
app_mode = st.sidebar.radio("請選擇操作", ["📡 美股版塊熱力圖", "📡 港股版塊熱力圖", "🔍 美股版塊尋龍掃描", "🚀 個股深度透視"])
                ...
                if len(df) > 63:
                    ...
                    if se_s > 85 and cej_s > 110 and crs_val > 52:
                        ...
                        <div class='scan-card scan-card-fire'>
                            <h2 style='color:#FFD700; margin:0;'>🎯 {t} | 能量共振！</h2>
                            <p style='margin:5px 0; color:#ddd;'>
                                🧬 DNA: <b>{dna_v}</b> | ⚡ SE: <b style='color:#FF4B4B;'>{se_s:.1f}</b> | 
                                🔋 EJ: <b style='color:#00FFCC;'>{cej_s:.1f}</b> | 📈 RS: <b>{crs_val:.1f}</b>
                            </p>
                        </div>
                        """, unsafe_allow_html=True)
                    else:
...
# ==========================================
    ...
    ticker = st.sidebar.text_input("🚀 輸入資產代號 (例: 0700.HK, NVDA)", "NVDA").upper()
        ...
        if not df.empty:
            ...
            with d_c1:
                ...
                <div class='cosmos-box' style='border-color:#FF4B4B; height:380px; display:flex; flex-direction:column; justify-content:center;'>
                    ...
                    <div style='font-size:0.9rem; opacity:0.7; margin:5px 0;'>{dna_title}</div>
                    <div style='font-size:6rem; font-weight:900;'>{dna_v}</div>
                    <div style='color:#FFD700; margin-top:10px;'>[ 現屬 {d_lv} ]<br><span style='font-size:1.6rem; color:#FFF;'>{d_desc}</span></div>
                </div>
                """, unsafe_allow_html=True)
            with d_c2:
            ...
            def v_card(col, title, t_val, f_val, desc):
                col.markdown(f"""
                <div class='val-box'>
                    <div class='val-label'>{title}</div>
                    <div class='val-text'>TTM: <span class='val-focus'>{t_val}</span></div>
                    <div class='val-text'>12M預期: <span class='val-focus'>{f_val}</span></div>
                    <div style='color:#FFA500; font-size:0.9rem; margin-top:10px;'>{desc}</div>
                </div>
                """, unsafe_allow_html=True)
            ...
            if ttm_pe > 80 and not is_etf:
                ...
                if dragon_index >= 80: t_lv, t_desc, val_title, val_color, act_desc = "第 1 級", "極致真龍", "🔥 烈火鳳凰", "#BC13FE", "【順勢而為】真實財報極度健康，估值雖貴但有強大動能支撐，緊貼趨勢操作。"
                elif dragon_index >= 65: t_lv, t_desc, val_title, val_color, act_desc = "第 2 級", "潛力金龍", "🌟 潛龍伏躍", "#00FFCC", "【價值防守】財報穩健，動能醞釀中，適合分批建倉或持有觀望。"
                elif dragon_index >= 40: t_lv, t_desc, val_title, val_color, act_desc = "第 3 級", "中庸凡骨", "⚠️ 海市蜃樓", "#FFA500", "【謹慎觀望】動能與財報表現平平，估值偏高，注意回調風險。"
                else: t_lv, t_desc, val_title, val_color, act_desc = "第 4 級", "高危泥鰍", "☠️ 末路狂花", "#FF4B4B", "【規避風險】財報轉弱且動能破位，估值存在泡沫，建議嚴格止損。"
