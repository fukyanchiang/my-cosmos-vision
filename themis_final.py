...
def get_cached_info(ticker_sym):
    try: return yf.Ticker(ticker_sym).info
    except: return {}

@st.cache_data(ttl=3600)
def get_holders(ticker_sym):
    try: return yf.Ticker(ticker_sym).institutional_holders
...
def smart_fetch(ticker_sym, period="1y"):
    ...
    try:
        ...
        if data.empty:
            time.sleep(1.0) 
            data = yf.download(ticker_sym, period=period, progress=False)
...
# 🔘 側邊欄控制 (V173.0)
st.sidebar.markdown("## 🛰️ 戰術控制台 (V173.0)")
...
# =========================================================================
# 🚀 模式 A：個股深度透視 (V173.0 終極修復版)
# =========================================================================
if app_mode == "🚀 個股深度透視":
    ticker = st.sidebar.text_input("🚀 輸入資產代號", "6869.HK").upper()
    analyze_btn = st.sidebar.button("🔍 確認透視")
    
    # 爺爺幫你加粒「記性」掣：
    if 'mode_a_ticker' not in st.session_state: st.session_state.mode_a_ticker = ""
    if analyze_btn: st.session_state.mode_a_ticker = ticker
    
    # 只有撳咗掣先會執行下面所有野：
    if st.session_state.mode_a_ticker != "":
        ...
        with st.spinner(f"⏳ 系統正在切換引擎，重新為您下載海量數據及繪製摩訶圖... 請稍候 ☕🚀"):
            try:
                ...
                b_sym_plot = "2800.HK" if ".HK" in ticker_to_use else "SPY"
                ...
                if not df.empty:
                    ...
                    st.markdown(f"""<div class='main-title' style='margin-bottom:10px;'>環球資產透維評估儀 [{ticker_to_use}]{name_html}</div>""", unsafe_allow_html=True)
                    ...
                    # 爺爺新股羅輯：唔夠 63 日就同上市第一日比
                    if len(df) > 0:
                        lookback_idx = -63 if len(df) >= 63 else 0
                        base_p = df['Close'].iloc[lookback_idx]
                        base_spy = spy_aligned.iloc[lookback_idx] if len(spy_aligned) > 0 else 1
                        crs_val = safe_n(50 + ((curr_p / base_p) - (spy_aligned.iloc[-1] / base_spy)) * 100, 50.0)
                    else:
                        crs_val = 50.0

                    v21 = df['Volume'].tail(21).mean(); v252 = df['Volume'].tail(252).mean(); cej_s = safe_n((v21 / max(v252, 1)) * 100, 50.0)
                    if len(df) > 0:
                        se_lookback = -5 if len(df) >= 5 else 0
                        se_s = safe_n(50 + (((curr_p / df['Close'].iloc[se_lookback]) - 1) * 1200), 50.0)
                    else:
                        se_s = 50.0
                    ...
                    def get_trend_stats(metric):
                        try:
                            is_new = len(df) < 40
                            if metric == "RS":
                                past_lookback = -20 if len(df) >= 20 else 0
                                past_bench_idx = -83 if len(df) >= 83 else 0
                                past_p = df['Close'].iloc[past_lookback]; past_spy = spy_aligned.iloc[past_lookback]
                                past_bench = df['Close'].iloc[past_bench_idx] 
                                past_bench_spy = spy_aligned.iloc[past_bench_idx] if len(spy_aligned) > 0 else 1
                                past = 50 + ((past_p / past_bench) - (past_spy / past_bench_spy)) * 100
                                diff = crs_val - past
                            elif metric == "EJ":
                                v_past_21 = df['Volume'].iloc[-41:-20].mean() if len(df) > 20 else df['Volume'].mean()
                                v_past_252 = df['Volume'].iloc[-273:-20].mean() if len(df) > 280 else (df['Volume'].iloc[:-20].mean() if len(df) > 20 else df['Volume'].mean())
                                past = (v_past_21 / max(v_past_252, 1)) * 100
                                diff = cej_s - past
                            else: 
                                p_lookback = -20 if len(df) >= 20 else 0
                                p_base = -25 if len(df) >= 25 else 0
                                past = 50 + (((df['Close'].iloc[p_lookback] / df['Close'].iloc[p_base]) - 1) * 1200)
                                diff = se_s - past
                            color = "#00FF00" if diff >= 0 else "#FF4B4B"
                            res_str = f"{'+' if diff>=0 else ''}{diff:.1f}%"
                            if is_new: res_str += " (累積中)"
                ...
                try:
                    ...
                    curr_tail = min(20, len(mf_df))
                    curr_20d_flow = mf_df['Net_Flow'].tail(curr_tail).sum()
                    prev_20d_flow = mf_df['Net_Flow'].iloc[-(curr_tail*2):-curr_tail].sum() if len(mf_df) > curr_tail else 0
                    ...
                    # 爺爺保險絲：OBV 百分比 Cap 死
                    mf_pct = min(999.0, max(-999.0, mf_pct))
                    
                    obv_curr_val = mf_df['OBV'].iloc[-1] - mf_df['OBV'].iloc[-curr_tail] if len(mf_df) >= curr_tail else mf_df['OBV'].iloc[-1] - mf_df['OBV'].iloc[0]
                    if len(mf_df) > curr_tail:
                        prev_idx = max(0, len(mf_df) - (curr_tail * 2))
                        obv_prev_val = mf_df['OBV'].iloc[-curr_tail] - mf_df['OBV'].iloc[prev_idx]
                    else:
                        obv_prev_val = 1
                    
                    price_trend = mf_df['Close'].iloc[-1] - mf_df['Close'].iloc[-curr_tail] if len(mf_df) >= curr_tail else mf_df['Close'].iloc[-1] - mf_df['Close'].iloc[0]
                    obv_pct = (obv_curr_val - obv_prev_val) / abs(obv_prev_val) * 100 if obv_prev_val != 0 else 0
                    obv_pct = min(999.0, max(-999.0, obv_pct))
                    
                    obv_total_vol = mf_df['Volume'].tail(curr_tail).sum() or 1
                    ...
                    daily_abs_flow = abs(mf_df['Net_Flow'].tail(curr_tail)); total_abs_flow = daily_abs_flow.sum() or 1; conc_pct = (daily_abs_flow.max() / total_abs_flow) * 100
                    
                    # 爺爺上市不足提醒：
                    data_warning = ""
                    if len(df) < 40:
                        data_warning = "<div style='color:#FFA500; text-align:center; margin-bottom:10px; font-weight:bold;'>⚠️ 數據累積中：此資產上市不足 40 日，部分資金變化比率乃與上市首日比較。</div>"
                    ...
                    if data_warning: st.markdown(data_warning, unsafe_allow_html=True)
    ...
    with st.spinner(f"⏳ 大宗師正在計算市寬數據... 請稍候 ☕🚀"):
        try:
            ...
            if not idx_df.empty:
                ...
                fig_trend.update_layout(template="plotly_dark", paper_bgcolor='#0e1117', plot_bgcolor='#111', height=300, margin=dict(l=20, r=60, t=10, b=20), xaxis=dict(type='category'), yaxis=dict(range=[0, 105]))
                st.plotly_chart(fig_trend, use_container_width=True, config={'displayModeBar': False})

                fig = make_subplots(rows=2, cols=1, shared_xaxes=True, row_heights=[0.7, 0.3], vertical_spacing=0.05)
                o_col, c_col, h_col, l_col, v_col = clean_recent['Open'].values, clean_recent['Close'].values, clean_recent['High'].values, clean_recent['Low'].values, clean_recent['Volume'].values
                if show_b_idx: fig.add_trace(go.Candlestick(x=dates, open=o_col, high=h_col, low=l_col, close=c_col, name='基準指數'), row=1, col=1)
                ...
                fig.add_trace(go.Bar(x=dates, y=v_col, name='成交量'), row=2, col=1)
                fig.update_layout(template="plotly_dark", paper_bgcolor='#0e1117', height=750, xaxis_rangeslider_visible=False)
                st.plotly_chart(fig, use_container_width=True)
        except Exception as e: st.error(f"⚠️ 數據載入失敗：{e}")

# =========================================================================
# 其餘模式保持原樣，僅確保使用緩存引擎 (C/D/E/F/G)
# =========================================================================
        ...
        with st.spinner("⏳ 慢速引擎過濾中..."):
            for idx, t in enumerate(tickers_to_scan):
                ...
                try:
                    ...
                    if len(d_full) > 100:
                        d = d_full.tail(63); curr_p = d['Close'].iloc[-1]; ath = d_full['High'].tail(252).max()
                        if t_strat == "🔥 極致新高 (ATH)":
                        ...
                        net_flow_20 = nf.tail(20).sum(); crs = 50+((curr_p/d['Close'].iloc[-63])-(bench_data['Close'].iloc[-1]/bench_data['Close'].iloc[-63]))*100 if len(d)>60 else 50
                        ...
                        if net_flow_20 > 0 and se > 75 and ej > 85 and crs > 52:
                            found = True; st.markdown(f"<div class='scan-card-fire'><h2>🎯 {t} | 符合大戶佈局！</h2><p>💰 資金流: {net_flow_20/1e8:.1f}億 | ⚡ SE: {se:.1f} | 🔋 EJ: {ej:.1f} | 📈 RS: {crs:.1f}</p></div>", unsafe_allow_html=True)
                except: pass
    ...
    with st.spinner('拔河排名計算中...'):
        try:
            ...
            if results:
                ...
                fig.update_layout(template="plotly_dark", paper_bgcolor='#0e1117', height=700)
                st.plotly_chart(fig, use_container_width=True)
        except: pass
    ...
    target_dict = (US_ETF_MAP if is_us else HK_ETF_MAP) if is_etf else (US_STOCK_MAP if is_us else HK_STOCK_MAP)
    with c_sec: s_choice = st.selectbox("3. 選擇範圍", ["🌐 全星系大規模搜索"] + list(target_dict.keys()))
    with c_strat: vcp_strat = st.radio("4. 戰術過濾", ["🔥 極致新高 (ATH)", "🐉 潛龍伏躍"])
    if 'vcp_scanned_stocks' not in st.session_state: st.session_state.vcp_scanned_stocks = []
    if st.button("📡 執行核心 RS 海選與大戶偵測"):
        tickers_to_scan = list(set([t for sub in target_dict.values() for t in sub])) if "全星系" in s_choice else target_dict[s_choice]
            ...
            try:
                for idx, t in enumerate(tickers_to_scan):
                    ...
                    try:
                        ...
                        if len(df_t) > 150:
                            curr, ath = df_t.iloc[-1], df_t['High'].tail(252).max()
                            if vcp_strat == "🔥 極致新高 (ATH)" and (curr['Close']/ath) < 0.93: continue
                            df_t['Vol50'] = df_t['Volume'].rolling(50).mean()
                            whale = len(df_t.tail(10)[(df_t.tail(10)['Close']>df_t.tail(10)['Open']) & (df_t.tail(10)['Volume']>df_t.tail(10)['Vol50']*1.5)])
                            found_stocks.append({'Ticker': t, 'RS Rating': 85, 'Tags': f"🔥 大戶掃貨 ({whale}/10)" if whale>=3 else "", 'Pivot': df_t['High'].tail(20).max()})
                    except: continue
                st.session_state.vcp_scanned_stocks = found_stocks
            except: pass
    if st.session_state.vcp_scanned_stocks:
        sel = st.selectbox("🎯 選擇目標查看戰術圖", [s['Ticker'] for s in st.session_state.vcp_scanned_stocks])
        if sel:
    ...
    with c3: turtle_strat = st.radio("3. 海龜戰術", ["🔥 極致真龍 (ATH 回測)", "🐉 潛龍初醒"])
    is_us = "美股" in market_choice; is_etf = "ETF" in asset_type
    target_dict = (US_ETF_MAP if is_etf else US_STOCK_MAP) if is_us else (HK_ETF_MAP if is_etf else HK_STOCK_MAP)
    s_choice = st.selectbox("4. 選擇範圍", ["🌐 全星系大規模搜索"] + list(target_dict.keys()))
        ...
        with st.spinner("過濾中..."):
            for idx, t in enumerate(tickers):
                ...
                try:
                    ...
                    if len(df) > 150:
                        curr_p, ma50 = df['Close'].iloc[-1], df['Close'].rolling(50).mean().iloc[-1]
                        ema10 = df['Close'].ewm(span=10, adjust=False).mean().iloc[-1]
                        if curr_p > ma50 and ema10*0.98 <= curr_p <= ema10*1.04:
                            found.append({'Ticker': t, 'Price': curr_p, 'Pullback': -5.0, 'EMA10': ema10, 'High': df['High'].tail(20).max(), 'Low': df['Low'].tail(20).min(), 'Days Since High': 5})
                except: continue
            st.session_state.e_scanned_stocks = found
    if st.session_state.e_scanned_stocks:
        sel = st.selectbox("🎯 選擇目標", [s['Ticker'] for s in st.session_state.e_scanned_stocks])
        if sel:
            df = smart_fetch(sel, period="6mo")
