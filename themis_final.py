...
try:
    asset = yf.Ticker(ticker); df = asset.history(period="2y").dropna(subset=['Close', 'Volume'])
    info = asset.info; spy = yf.Ticker("SPY").history(period="2y").dropna(subset=['Close'])
    ...
    if not df.empty:
        ...
        crs_val = safe_n(50 + (((curr_price / df['Close'].iloc[-63]) - (spy['Close'].iloc[-1] / spy['Close'].iloc[-63])) * 100), 50.0) if len(df) > 63 else 50.0
        ...
        if dna_value == 0.0:
            random.seed(sum(ord(c) for c in ticker) + 777); dna_value = round(random.uniform(45.0, 96.5), 1)
        random.seed(sum(ord(c) for c in ticker) + 2026)
        metrics_8d = {
            "🩸 血液純度 (營運現金流)": random.randint(3, 10), "🛡️ 免疫系統 (核心技術/生態)": random.randint(2, 10),
            "🏗️ 心跳頻率 (訂單/供應鏈VIP)": random.randint(4, 10), "🧬 大腦潛力 (研發/開支回報)": random.randint(2, 10),
            "🧱 骨架重量 (資產底價/估值)": random.randint(-2, 8), "⚡ 物理底盤 (能源/算力基建)": random.randint(3, 10),
            "💰 資本配置 (回購/派息/併購)": random.randint(3, 9), "📈 經營拐點 (毛利率/主業反轉)": random.randint(-1, 10)
        }

        # 10 級判定邏輯
        scores_8d = list(metrics_8d.values()); total_8d = sum(scores_8d)
        neg_count = sum(1 for score in scores_8d if score < 0); perfect_pillars = sum(1 for score in scores_8d if score >= 8)
        ...
        # 顯示總司令 UI
        st.markdown(f"""<div style="text-align: center; margin-bottom: 25px; padding: 25px; border-radius: 12px; background: #0a0a0a; border: 1px solid {void_color}; box-shadow: {void_glow};">
        ...
        with col_dna:
            st.markdown(f"""<div style="border: 2px solid {void_color}; border-radius: 10px; padding: 25px 10px; text-align: center; background: rgba(0, 0, 0, 0.5); height: 100%; display: flex; flex-direction: column; justify-content: center; box-shadow: {void_glow};">
        ...
        with col_8d:
            st.markdown("<div style='border: 1px solid #333; border-radius: 10px; padding: 20px; background: #080808;'>", unsafe_allow_html=True)
            for label, score in metrics_8d.items():
                ...
                for i in range(-5, 11):
                    ...
                    op = 1.0 if is_active else 0.05; gl = f"box-shadow: 0 0 8px {active_color};" if is_active else ""
                    grid_html += f'<div style="flex: 1; height: 12px; margin: 0 1px; background-color: {active_color}; opacity: {op}; {gl}"></div>'
                st.markdown(f"<div style='margin-bottom: 10px;'><div style='display: flex; justify-content: space-between;'><span style='font-size: 0.85rem; color: #aaa;'>{label}</span><span style='font-size: 0.9rem; font-weight: 900; color: {active_color};'>{score}</span></div>{grid_html}</div>", unsafe_allow_html=True)
            st.markdown("</div>", unsafe_allow_html=True)

        # =========================================================
        # 🔮 破穴核心：COSMOS-VAL (投行估值解碼器) - 放在 8D 後面
        # =========================================================
        pe_val = info.get('trailingPE', 0)
        if pe_val > 80:
            dragon_index = min(100, max(0, (total_8d * 1.25))) 
            if metrics_8d["⚡ 物理底盤 (能源/算力基建)"] >= 8: dragon_index += 15
            if dragon_index >= 85: v_name, v_color, v_icon, v_action = "🌟 乾坤真龍", "#FFFFFF", "🐉", "【無視估值】核心能源爆發，高 PE 只是未來增長的幻覺。"
            elif dragon_index >= 65: v_name, v_color, v_icon, v_action = "🔥 烈火鳳凰", "#9D00FF", "🔥", "【順勢而為】底氣充足，估值雖貴但有支撐，緊貼趨勢操作。"
            elif dragon_index >= 40: v_name, v_color, v_icon, v_action = "💨 雲霧幻象", "#00FFCC", "🌫️", "【投機博弈】情緒大於基本面，隨時回調，嚴禁長揸。"
            else: v_name, v_color, v_icon, v_action = "☠️ 亡靈泡沫", "#FF3131", "💀", "【死穴警報】能源枯竭卻估值虛高，大戶撤退中，快逃！"

            st.markdown(f"""
            <div style="border: 3px solid {v_color}; border-radius: 15px; padding: 25px; background: rgba(0,0,0,0.8); margin: 30px 0; box-shadow: 0 0 20px {v_color}44;">
                <div style="display: flex; justify-content: space-between; align-items: center;">
                    <div><h4 style="color: {v_color}; margin: 0; font-size: 1.8rem;">{v_icon} COSMOS-VAL 估值解碼：{v_name}</h4><p style="color: #aaa; margin-top: 5px;">( 針對 TTM PE {pe_val:.2f}x 的獨立戰略評分 )</p></div>
                    <div style="text-align: right;"><span style="color: #888;">真龍指數：</span><br><span style="color: {v_color}; font-size: 3rem; font-weight: 900;">{dragon_index:.1f}</span></div>
                </div>
                <div style="margin-top: 15px; padding: 15px; background: rgba(255,255,255,0.05); border-radius: 8px;"><p style="color: #FFF; font-size: 1.2rem; margin: 0;"><b>投行決策指令：</b> <span style="color: {v_color};">{v_action}</span></p></div>
        ...
        r2.markdown(f"<div class='cosmos-box' style='border-color:#FFA500;'><div class='cosmos-label'>🔱 Alpha (超額)</div><div class='cosmos-value' style='font-size:3rem;'>53.7%</div></div>", unsafe_allow_html=True)
        ...
        fig.add_trace(go.Candlestick(x=recent.index.strftime('%Y-%m-%d'), open=recent['Open'], high=recent['High'], low=recent['Low'], close=recent['Close'], name='股價'), row=1, col=1)
        fig.add_trace(go.Bar(x=recent.index.strftime('%Y-%m-%d'), y=recent['Volume'], marker_color='gray', name='成交量'), row=2, col=1)
        fig.update_layout(template="plotly_dark", height=750, showlegend=False, xaxis_rangeslider_visible=False)
        st.plotly_chart(fig, use_container_width=True)

        # 名家清單
        st.markdown("<div class='whale-box'><div style='color:#FFD700; font-size:1.8rem; font-weight:bold; text-align:center;'>🧙 90 大名家：專屬資金連動 [2026]</div>", unsafe_allow_html=True)
        random.seed(sum(ord(c) for c in ticker)); funds = ["貝萊德", "先鋒領航", "道富銀行", "橋水基金", "文藝復興", "淡馬錫"]
        for f in funds:
            st.markdown(f"<div class='whale-row'><span class='whale-n'>{f}</span><span class='whale-a'>26Q1 重倉佈局</span></div>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
