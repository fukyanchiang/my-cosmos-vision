...
# =========================================================================
    ...
    if scan_btn:
        ...
        progress_bar.empty()
        if not breakout_found: st.warning("💤 掃描完畢：目前未有 ETF 觸發起飛訊號。")import streamlit as st 
import yfinance as yf 
import pandas as pd 
import numpy as np 
import plotly.graph_objects as go 
...
if app_mode == "🚀 個股深度透視":
    ...
    try:
        ...
        if not df.empty:
            ...
            with d_c1: st.markdown(f"<div class='cosmos-box' style='border-color:#FF4B4B; height:380px; display:flex; flex-direction:column; justify-content:center;'><div style='color:#FF4B4B; font-weight:900; font-size:1.8rem;'>🧬 COSMOS-DNA</div><div style='font-size:0.9rem; opacity:0.7; margin:5px 0;'>{dna_title} (100分滿分)</div><div style='font-size:6rem; font-weight:900;'>{dna_v}</div><div style='color:#FFD700; font-size:1rem; font-weight:bold; margin-top:10px;'>[ 註明：共分 10 級，現屬 {d_lv} ]<br><span style='font-size:1.6rem; color:#FFF;'>{d_desc}</span></div></div>", unsafe_allow_html=True)
            with d_c2:
                st.markdown(f"**{ticker} ・ 8D 投行精確透視 BAR**")
                colors_8d = ["#00FFCC", "#00FFCC", "#00FFCC", "#00FFCC", "#FF4B4B", "#BC13FE", "#FFFFFF", "#FFD700"]
            ...
            # 估值矩陣 & 烈火鳳凰
            st.write(""); k1 = st.columns(4); k2 = st.columns(4)
            val_emotion = safe_n(crs_val * 0.9, 50.0); val_total = (cx_val + crs_val + se_s) / 3; val_vol_ratio = v21 / max(v252, 1)
            target_p = info.get('targetMeanPrice'); val_target_str = f"${target_p:.2f}" if target_p else "N/A"
            kings = [("📁 質量", f"{dna_v:.0f}"), ("📈 趨勢", f"{crs_val:.0f}"), ("⚡ 動能", f"{se_s:.0f}"), ("🔋 大資金", f"{cej_s:.0f}"), ("🎭 情緒", f"{val_emotion:.0f}"), ("🏆 總分", f"{val_total:.0f}"), ("🔮 12M目標", val_target_str), ("💰 成交比", f"{val_vol_ratio:.1f}x")]
            for i in range(4):
                k1[i].markdown(f"<div class='cosmos-box' style='padding:15px; border-width:2px;'><div style='color:#ccc; font-size:1.2rem;'>{kings[i][0]}</div><div style='color:#FFD700; font-size:2.5rem; font-weight:bold;'>{kings[i][1]}</div></div>", unsafe_allow_html=True)
                k2[i].markdown(f"<div class='cosmos-box' style='padding:15px; border-width:2px; border-color:#FFD700;'><div style='color:#ccc; font-size:1.2rem;'>{kings[i+4][0]}</div><div style='color:#FFD700; font-size:2.5rem; font-weight:bold;'>{kings[i+4][1]}</div></div>", unsafe_allow_html=True)
            ...
            st.write("### 🏛️ 估值與風險全方位透視")
            v1, v2, v3 = st.columns(3); v4, v5, v6 = st.columns(3)
            def v_card(col, title, t_v, f_v, desc): col.markdown(f"<div class='val-box'><div class='val-label'>{title}</div><div class='val-text'>TTM: <span class='val-focus'>{t_v}</span></div><div class='val-text'>12M預期: <span class='val-focus'>{f_v}</span></div><div style='color:#FFA500; font-size:0.9rem; margin-top:10px;'>{desc}</div></div>", unsafe_allow_html=True)
            v_card(v1, "PE 獲利比", safe_s(info, ['trailingPE'], "x"), safe_s(info, ['forwardPE'], "x"), "獲利估值透視")
            v_card(v2, "PEG 增長比", safe_s(info, ['pegRatio']), "N/A", "增長性價比")
            v_card(v3, "PS 營收比", safe_s(info, ['priceToSalesTrailing12Months'], "x"), "N/A", "營收規模")
            v_card(v4, "PB 淨資產", safe_s(info, ['priceToBook'], "x"), "N/A", "賬面價值")
            ...
            # 烈火鳳凰邏輯
            ttm_pe = info.get('trailingPE', 0) or 0; fwd_pe = info.get('forwardPE', 0) or 0
            if not is_etf:
                dragon_index = round((dna_v * 0.4) + (cx_val * 0.3) + (crs_val * 0.3), 1)
                dragon_index = max(5.0, min(98.5, dragon_index)) 
                if dragon_index >= 80: t_lv, t_desc, val_title, val_color = "第 1 級", "極致真龍", "🔥 烈火鳳凰", "#BC13FE"; act_desc = "【順勢而為】真實財報極度健康，估值雖貴但有強大動能支撐，緊貼趨勢操作。"
                elif dragon_index >= 65: t_lv, t_desc, val_title, val_color = "第 2 級", "潛力金龍", "🌟 潛龍伏躍", "#00FFCC"; act_desc = "【價值防守】財報穩健，動能醞釀中，適合分批建倉或持有觀望。"
                elif dragon_index >= 40: t_lv, t_desc, val_title, val_color = "第 3 級", "中庸凡骨", "⚠️ 海市蜃樓", "#FFA500"; act_desc = "【謹慎觀望】動能與財報表現平平，估值偏高，注意回調風險。"
                else: t_lv, t_desc, val_title, val_color = "第 4 級", "高危泥鰍", "☠️ 末路狂花", "#FF4B4B"; act_desc = "【規避風險】財報轉弱且動能破位，估值存在泡沫，建議嚴格止損。"
                warning_html = "<span style='color:#FF0000; font-size:3.5rem; font-weight:900; margin-left:20px; text-shadow: 2px 2px 4px #000;'>警告</span>" if (ttm_pe > 80 or fwd_pe > 80) else ""
                st.markdown(f"<div style='border: 4px solid {val_color}; border-radius: 15px; padding: 30px; background-color: #000; box-shadow: 0 0 30px {val_color}66; margin: 25px 0;'><div style='display:flex; justify-content:space-between; align-items:center;'><div><span style='font-size:2.2rem; font-weight:900;'>COSMOS-VAL 解碼：<span style='color:{val_color};'>{val_title}</span>{warning_html}</span><br><span style='font-size:1.1rem; opacity:0.8;'>（針對 TTM PE {ttm_pe:.2f}x 獨立戰術評分）</span><br><span style='font-size:1.2rem; color:#FFD700; font-weight:bold; margin-top:5px; display:inline-block;'>[ 註明：共分 4 級，現在這公司基於真實財報屬 {t_lv} ({t_desc}) ]</span></div><div style='text-align:right;'><span style='font-size:1.6rem;'>真龍指數：</span><br><span style='font-size:5rem; font-weight:900; color:{val_color};'>{dragon_index}</span></div></div><div style='background-color:#111; padding:20px; border-radius:10px; margin-top:20px; border:1px solid #333;'><b style='color:white; font-size:1.3rem;'>真實財報決策指令：</b> <span style='color:{val_color}; font-size:1.3rem;'>{act_desc}</span></div></div>", unsafe_allow_html=True)

            # Alpha/波動率
            b_val = float(get_beta(info, df, spy)); y1_r = (curr_p / df['Close'].iloc[-252] - 1) if len(df) > 252 else 0; s_y1_r = (spy['Close'].iloc[-1] / spy['Close'].iloc[-252] - 1) if len(spy) > 252 else 0; real_alpha = (y1_r - b_val * s_y1_r) * 100
            r1, r2, r3 = st.columns(3)
            r1.markdown(f"<div class='cosmos-box' style='border-color:#FFA500;'><div class='cosmos-label'>📐 Beta (性格)</div><div class='cosmos-value' style='font-size:3.5rem;'>{b_val:.2f}</div></div>", unsafe_allow_html=True)
            r2.markdown(f"<div class='cosmos-box' style='border-color:#FFA500;'><div class='cosmos-label'>🔱 Alpha (超額)</div><div class='cosmos-value' style='font-size:3.5rem;'>{real_alpha:.1f}%</div></div>", unsafe_allow_html=True)
            r3.markdown(f"<div class='cosmos-box' style='border-color:#FFA500;'><div class='cosmos-label'>🌊 波動率 (情緒)</div><div class='cosmos-value' style='font-size:3.5rem;'>{(v_ann*100):.1f}%</div></div>", unsafe_allow_html=True)
            ...
            if holders is not None and not holders.empty and 'Holder' in holders.columns:
                for _, row in holders.head(8).iterrows():
                    shares = row.get('Shares', 0); calc_pct = (shares / total_shares) if total_shares > 1 else 0; val_m = row.get('Value', 0) / 1e6
                    st.markdown(f"<div class='whale-row'><span class='whale-n'>{row['Holder']}</span><span class='whale-a'>持有 {shares:,.0f} 股 | 佔比 {calc_pct:.2%} | 市值 ${val_m:.1f}M</span></div>", unsafe_allow_html=True)
...
# =========================================================================
    ...
    st.write("👴 爺爺提示：此處專注比較**個股板塊**，尋找資金炒作的熱點。圖表已鎖定，可用手指點擊查看數值。")
    m_view = st.sidebar.radio("選擇星系地圖", ["🇺🇸 美股 50 大版塊", "🇭🇰 港股 22 大版塊"])
    is_us = "美股" in m_view; bench_sym = "SPY" if is_us else "^HSI"; target_map = US_STOCK_MAP if is_us else HK_STOCK_MAP
    with st.spinner('拔河中...'):
        try:
            ...
            for name, tickers in target_map.items():
                for t in tickers:
                    try:
                        ...
                        if len(d) >= 20:
                            rs_score = 50 + ((d.iloc[-1]/d.iloc[-20]) - (bench_df.iloc[-1]/bench_df.iloc[-20])) * 100
                            if not np.isnan(rs_score) and not np.isinf(rs_score): results.append({"版塊": name, "RS強弱": round(rs_score, 1)}); break
                    except: continue
            if results:
                df_rs = pd.DataFrame(results).sort_values("RS強弱", ascending=True)
                fig = go.Figure(go.Bar(x=df_rs["RS強弱"], y=df_rs["版塊"], orientation='h', marker=dict(color=df_rs["RS強弱"], colorscale='Portland' if is_us else 'Viridis')))
                fig.update_layout(template="plotly_dark", paper_bgcolor='#0e1117', plot_bgcolor='#0e1117', font=dict(color='white'), height=1000 if is_us else 700, title=f"最強個股吸金版塊：{df_rs.iloc[-1]['版塊']}", xaxis=dict(fixedrange=True, showgrid=False), yaxis=dict(fixedrange=True))
                st.plotly_chart(fig, use_container_width=True, theme=None, config={'displayModeBar': False})
        except: st.error("大盤數據暫缺")

# =========================================================================
# 📡 模式 B2：ETF 資產拔河熱力圖 (鎖定防誤觸 + 黑底 + 統一顏色)
# =========================================================================
elif app_mode == "📡 ETF 資產拔河熱力圖":
    st.markdown("<h1 class='main-title'>📡 ETF 資產相對強弱拔河排名</h1>", unsafe_allow_html=True)
    st.write("👴 爺爺提示：此處展示**大類資產與國家 ETF** 排名，判斷避險與進攻大方向。圖表已鎖定，可用手指點擊查看數值。")
    m_view = st.sidebar.radio("選擇星系地圖", ["🇺🇸 美股 ETF 陣列", "🇭🇰 港股 (含A股) ETF 陣列"])
    is_us = "美股" in m_view; bench_sym = "SPY" if is_us else "^HSI"; target_map = US_ETF_MAP if is_us else HK_ETF_MAP
    with st.spinner('拔河中...'):
        try:
            bench_df = yf.Ticker(bench_sym).history(period="60d")['Close'].dropna(); results = []
            for name, tickers in target_map.items():
            ...
            if results:
                df_rs = pd.DataFrame(results).sort_values("RS強弱", ascending=True)
                fig = go.Figure(go.Bar(x=df_rs["RS強弱"], y=df_rs["版塊"], orientation='h', marker=dict(color=df_rs["RS強弱"], colorscale='Portland' if is_us else 'Viridis')))
                fig.update_layout(template="plotly_dark", paper_bgcolor='#0e1117', plot_bgcolor='#0e1117', font=dict(color='white'), height=600, title=f"最強 ETF 資產：{df_rs.iloc[-1]['版塊']}", xaxis=dict(fixedrange=True, showgrid=False), yaxis=dict(fixedrange=True))
                st.plotly_chart(fig, use_container_width=True, theme=None, config={'displayModeBar': False})
        except: st.error("大盤數據暫缺")
...
# =========================================================================
    ...
    st.markdown("<h1 class='main-title'>🔍 個股專屬：起步狙擊雷達</h1>", unsafe_allow_html=True)
    st.write("👴 爺爺提示：此雷達**專門過濾個股**，尋找資金與動能共振的爆發點！")
    m_choice = st.sidebar.selectbox("市場", ["🇺🇸 美股市場 (~2800隻)", "🇭🇰 港股市場 (~1500隻)"])
    target_dict = US_STOCK_MAP if "美股" in m_choice else HK_STOCK_MAP
    s_choice = st.sidebar.selectbox("狙擊版塊", ["🌐 啟動全星系個股盲掃"] + list(target_dict.keys()))
    if st.sidebar.button("📡 發射個股尋龍電波！"):
        bench_data = yf.Ticker("SPY" if "美股" in m_choice else "^HSI").history(period="2y").dropna()
        tickers_to_scan = list(set([t for sub in target_dict.values() for t in sub])) if s_choice == "🌐 啟動全星系個股盲掃" else target_dict[s_choice]
        if s_choice == "🌐 啟動全星系個股盲掃": st.warning("⚠️ 爺爺警告：全星系盲掃包含過千隻股票，為免網絡塞車，請耐心等候幾分鐘！")
        breakout_found = False; progress_bar = st.progress(0)
        for idx, t in enumerate(tickers_to_scan):
            progress_bar.progress((idx + 1) / len(tickers_to_scan))
            try:
                d = yf.Ticker(t).history(period="1y").dropna()
                if len(d) > 63:
                    curr_p = d['Close'].iloc[-1]; crs = 50 + ((curr_p / d['Close'].iloc[-63]) - (bench_data['Close'].iloc[-1] / bench_data['Close'].iloc[-63])) * 100
                    ej = (d['Volume'].tail(21).mean() / max(d['Volume'].tail(252).mean(), 1)) * 100; se = 50 + (((curr_p / d['Close'].iloc[-5]) - 1) * 1200)
                    if se > 85 and ej > 110 and crs > 52:
                        breakout_found = True
                        st.markdown(f"<div class='scan-card-fire'><h2 style='color:#FFD700; margin:0;'>🎯 {t} | 個股起飛訊號！</h2><p style='margin:5px 0; color:#ddd; font-size:1.2rem;'>⚡ SE: <b style='color:#FF4B4B;'>{se:.1f}</b> | 🔋 EJ: <b style='color:#00FFCC;'>{ej:.1f}</b> | 📈 RS: <b>{crs:.1f}</b></p></div>", unsafe_allow_html=True)
            except: pass
        progress_bar.empty()
        if not breakout_found: st.warning("💤 掃描完畢：目前未有標的觸發起飛訊號。")
...
# 🛡️ 模式 D1 & D2：ETF 專屬雷達
# =========================================================================
elif app_mode in ["🛡️ 美股 ETF 專屬雷達", "🛡️ 港/A股 ETF 專屬雷達"]:
    is_us = "美股" in app_mode
    st.markdown(f"<h1 class='main-title'>{app_mode}</h1>", unsafe_allow_html=True)
    target_dict = US_ETF_MAP if is_us else HK_ETF_MAP
    s_choice = st.sidebar.selectbox("選擇 ETF 版塊", ["🌐 啟動全 ETF 綜合掃描"] + list(target_dict.keys()))
    if st.sidebar.button("📡 發射 ETF 電波！"):
        bench_data = yf.Ticker("SPY" if is_us else "^HSI").history(period="2y").dropna()
        tickers_to_scan = list(set([t for sub in target_dict.values() for t in sub])) if s_choice == "🌐 啟動全 ETF 綜合掃描" else target_dict[s_choice]
        breakout_found = False; progress_bar = st.progress(0)
        for idx, t in enumerate(tickers_to_scan):
            progress_bar.progress((idx + 1) / len(tickers_to_scan))
            try:
                ...
                if len(d) > 63:
                    ...
                    if se > 85 and ej > 110 and crs > 52:
                        breakout_found = True
                        st.markdown(f"<div class='scan-card-fire' style='border-color:#00FFCC; box-shadow: 0 0 15px #00FFCC66;'><h2 style='color:#00FFCC; margin:0;'>🛡️ {t} | ETF 穩步發車！</h2><p style='margin:5px 0; color:#ddd;'>⚡ SE: <b style='color:#FF4B4B;'>{se:.1f}</b> | 🔋 EJ: <b style='color:#00FFCC;'>{ej:.1f}</b> | 📈 RS: <b>{crs:.1f}</b></p></div>", unsafe_allow_html=True)
            except: pass
        progress_bar.empty()
        if not breakout_found: st.warning("💤 掃描完畢：目前未有 ETF 觸發起飛訊號。")
