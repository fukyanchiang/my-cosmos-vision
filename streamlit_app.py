...
try:
    ...
    if not df.empty:
        ...
        for i in range(4):
            k1[i].markdown(f"<div class='cosmos-box' style='padding:15px; border-width:2px;'><div style='color:#ccc; font-size:1.4rem;'>{kings[i][0]}</div><div style='color:#FFD700; font-size:2.8rem; font-weight:bold;'>{kings[i][1]}</div></div>", unsafe_allow_html=True)
        ...
        def v_card(col, title, t_val, f_val, desc):
            col.markdown(f"<div class='val-box'><div class='val-label'>{title}</div><div class='val-text'>TTM: <span class='val-focus'>{t_val}</span></div><div class='val-text'>2026預準: <span class='val-focus'>{f_val}</span></div><div style='color:#FFA500; font-size:1.1rem; margin-top:10px;'>{desc}</div></div>", unsafe_allow_html=True)
        ...
        v_card(v3, "PS 營收比", safe_s(info, ['priceToSalesTrailing12Months'], suffix="x"), "2.9x", "營營規模")
        ...
        # 第四層：Beta/Alpha/波動率 (中文解說大字版)
        r1, r2, r3 = st.columns(3)
        r1.markdown(f"<div class='cosmos-box' style='border-color:#FFA500;'><div class='cosmos-label'>📐 Beta (性格)</div><div class='cosmos-value' style='font-size:3.5rem;'>{safe_s(info,['beta'])}</div><div style='color:#aaa; font-size:1.2rem;'>市場同步率：1.0為基準</div></div>", unsafe_allow_html=True)
        r2.markdown(f"<div class='cosmos-box' style='border-color:#FFA500;'><div class='cosmos-label'>🔱 Alpha (超額)</div><div class='cosmos-value' style='font-size:3.5rem;'>53.7%</div><div style='color:#aaa; font-size:1.2rem;'>贏過大盤之能力</div></div>", unsafe_allow_html=True)
        r3.markdown(f"<div class='cosmos-box' style='border-color:#FFA500;'><div class='cosmos-label'>🌊 波動率 (情緒)</div><div class='cosmos-value' style='font-size:3.5rem;'>{(v_ann*100):.1f}%</div><div style='color:#aaa; font-size:1.2rem;'>年化資產震盪頻率</div></div>", unsafe_allow_html=True)
        ...
        try:
            ...
            fig.update_layout(template="plotly_dark", paper_bgcolor='#0e1117', plot_bgcolor='#0e1117', height=850, showlegend=False, xaxis_rangeslider_visible=False, xaxis2=dict(overlaying='x', side='top', range=[0, max(counts)*6], showgrid=False, showticklabels=False))
            st.plotly_chart(fig, use_container_width=True)
        except: st.warning("股價圖載入中...")
