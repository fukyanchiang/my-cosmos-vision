...
HK_STOCK_MAP = {
    ...
    # (為咗節省空間顯示，爺爺寫咗 5 個板塊，你實際用可以加返足 22 個)
}
...
if st.session_state.page == 'HOME':
    ...
    elif st.session_state.target == 'ATH':
        st.info("🔥 已啟動「歷史新高」極致獵龍模式 (將掃描全港股)")
        is_ath_mode = True

    # --- 啟動雷達 ---
    if btn_radar:
        if st.session_state.target == 'SINGLE' and hasattr(st.session_state, 'single_ticker'):
            selected_tickers = [(st.session_state.single_ticker, "自選個股")]
            market_mode = "US" if not st.session_state.single_ticker.endswith(".HK") else "HK"
        elif st.session_state.target in ['HK', 'ATH']:
            unique_map = {}
        ...
        if selected_tickers:
            ...
            for i, (t, sec) in enumerate(selected_tickers):
                ...
                if is_ath_mode and not df.empty:
                    ...
                    if (curr_p / ath) < 0.93: continue # 唔喺新高附近，直接飛走
    ...
    if hasattr(st.session_state, 'dragon_results'):
        ...
        if chart_t:
            ...
            if not df_c.empty:
                ...
                fig.add_trace(go.Scatter(x=df_c.index, y=df_c['Close'].rolling(50).mean(), mode='lines', name='50MA', line=dict(color='yellow', width=1.5)), row=1, col=1)
