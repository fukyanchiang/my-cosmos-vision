...
HK_ETF_MAP = {
    "H1. A股門戶/旗艦大盤": "2822.HK 3188.HK 3109.HK 2823.HK 2846.HK 3147.HK 2801.HK 3010.HK 3081.HK 3151.HK 3072.HK 3042.HK 2839.HK 3180.HK 2827.HK 3139.HK 3118.HK 2838.HK 2800.HK".split(),
        ...
        try:
            ...
            if not df.empty:
                ...
                if len(c_tail) > 15:
                    ...
                    raw_val = 75.0 + (((slope * 252) / avg_p / v_std_ann) * 50)
                    cx_val = safe_n(raw_val, 50.0)
                    cx_val = max(5.0, cx_val) 
                else:
                    cx_val = 50.0
...
# =========================================================================
# 🔍 模式 C/F/G：起步尋龍雷達 (🔥 已經完美植入 5.0 大腦)
# =========================================================================
elif "雷達" in app_mode and "Mode E" not in app_mode:
    st.markdown(f"<h1 class='main-title'>{app_mode} (5.0 引擎啟動)</h1>", unsafe_allow_html=True)
    ...
    with c_sec: 
        s_choice = st.selectbox("2. 選擇掃描範圍", ["🌐 啟動全星系大規模搜索", "🎯 單一隻個股掃描"] + list(target_dict.keys()))
        if s_choice == "🎯 單一隻個股掃描":
            single_ticker = st.text_input("輸入股票代號 (例: NVDA, 0700.HK)").upper().strip()
        else:
            single_ticker = ""
            
    with c_strat: 
        t_strat = st.radio("3. 戰術過濾 (機變)", ["🔥 極致新高 (ATH)", "🐉 潛龍伏躍 (不限ATH)"])
    
    if 'c_scanned_stocks' not in st.session_state:
        st.session_state.c_scanned_stocks = []

    if st.button("📡 發射 5.0 明暗雙線尋龍電波！"):
        if s_choice == "🎯 單一隻個股掃描":
            if single_ticker:
        ...
        if tickers_to_scan:
            pb = st.progress(0)
            with st.spinner(f"⏳ 5.0 引擎正在極速審判 {len(tickers_to_scan)} 隻標的..."):
                for idx, t in enumerate(tickers_to_scan):
                    pb.progress((idx + 1) / len(tickers_to_scan))
                    if idx > 0 and idx % 20 == 0: time.sleep(0.5)
                    try:
                        ...
                        if t_strat == "🔥 極致新高 (ATH)":
                            ...
                            if (df['Close'].iloc[-1] / ath) < 0.93: continue
                            
                        if check_stop_loss(df): sl_list.append(t)
                        
                        # 呼叫 5.0 大腦
                        res = scan_dragon_logic(df, t, s_choice, market_mode)
                        if res: found_stocks.append(res)
                    except: pass
                    
            st.session_state.c_scanned_stocks = sorted(found_stocks, key=lambda x: x['Score'], reverse=True)
            
    if st.session_state.c_scanned_stocks:
        st.success(f"🎉 成功尋獲 {len(st.session_state.c_scanned_stocks)} 隻過關真龍！")
        for r in st.session_state.c_scanned_stocks:
            st.markdown(f"""
                <div class='scan-card-fire' style='border-left: 5px solid #00FFCC; background-color: #111; padding: 15px; margin-bottom: 10px; border-radius: 8px;'>
                    <div style='font-size:1.5rem;font-weight:bold;color:white;'>{r['Status']} {r['Ticker']} {r['Icons']}</div>
                    <div style='font-size:1.1rem; color:#CCCCCC; margin-top:8px;'>
                        <b>戰術總分: {r['Score']}分</b> | 
                        <b style='color:#FFD700;'>原始戰力: {r.get('RawPower', 0)} 🔥</b> | 
                        <b style='color:#FF4B4B;'>扣分: {r.get('Penalty', 0)} 🛑</b> | 
                        <span style='color:#FF4B4B; font-weight:bold;'>🛑 止損(10-EMA): ${r.get('EMA10', 0)}</span> | Bias: {r.get('Bias', 0)}%<br>
                        📈 RS: {r.get('RS', 0)} | 🔋 EJ: {r.get('EJ', 0)} | ⚡ SE: {r.get('SE', 0)} | 🌊 OBV: {r.get('OBV', '')} | 💰 資金流: {r.get('Flow', '')} | 🎯 集中度: {r.get('Conc', '')} | 🔥 買盤力: {r.get('Power', 0)}x
                    </div>
                </div>
            """, unsafe_allow_html=True)
        ...
        if sel_target:
            with st.spinner("正在為您繪製戰術圖表 (修復版)..."):
                try:
