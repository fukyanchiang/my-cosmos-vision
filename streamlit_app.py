...
# ================= 爺爺修正：核心動能與EJ底氣 (PART A 醫好0688) =================
            ...
            try:
                ...
                if not recent.empty:
                    ...
                    stars = recent[(recent['Close'] > recent['Open']) & (recent['Volume'] > recent['Vol50'] * 1.5)]
...
# =========================================================================
# 🔍 模式 C/F/G：起步尋龍雷達 (🌟 復原：縮放解鎖 + 星星 + HVN)
# =========================================================================
        ...
        if sel_target:
            with st.spinner("正在為您繪製戰術圖表..."):
                try:
                    ...
                    max_c = max(counts) if len(counts) > 0 and max(counts) > 0 else 1
                    fig.add_trace(go.Bar(y=(bins[:-1]+bins[1:])/2, x=counts, orientation='h', marker_color='rgba(150,150,150,0.3)', name='重貨區', hoverinfo='skip', xaxis='x6', yaxis='y1'), row=1, col=1)
