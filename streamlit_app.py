...
# --- 5. 龍魂神殿專頁 ---
    ...
    if nav[1].button("🇭🇰 港股 (全陣列)"): st.session_state.target = 'HK_STOCK'
    if nav[2].button("🇺🇸 美股 (CSV)"): st.session_state.target = 'US_CSV'
    if nav[3].button("🔍 個股"): st.session_state.target = 'SINGLE'
    if nav[4].button("📦 ETF (全陣列)"): st.session_state.target = 'ETF'
    ...
    if st.session_state.target == 'US_CSV':
        st.write("### 🇺🇸 選擇美股戰略名單：")
        m = st.columns(5)
        files = [("SP500_Equities.csv", "大藍籌"), ("Market_Focus.csv", "精選名單"), ("Industry_Focus.csv", "行業板塊"), ("Core_Stocks.csv", "核心"), ("US_ETFs.csv", "戰略ETF")]
        for i, (f, name) in enumerate(files):
            if m[i].button(f"選定 {name}"): 
                st.session_state.active_file = f
                st.success(f"✅ 已選定 {name} ({f})，請按右上角『📡 啟動雷達』")

    # --- ETF 選擇 ---
    elif st.session_state.target == 'ETF':
        st.write("### 📦 選擇 ETF 戰場：")
        e1, e2 = st.columns(2)
        if e1.button("🇭🇰 設定為：掃描港股 ETF"): st.session_state.active_file = 'HK_ETF'; st.success("✅ 已選定 港股 ETF (共 113 隻)")
    ...
    if btn_radar:
        ...
        if st.session_state.target == 'HK_STOCK':
            # 讀取 HK_STOCK_MAP 字典內所有的股票
            selected_tickers = [(t, k.split('.')[1].strip() if '.' in k else k) for k, v in HK_STOCK_MAP.items() for t in v]
            market_mode = "HK"
        elif st.session_state.target == 'ETF':
            if st.session_state.active_file == 'HK_ETF':
                selected_tickers = [(t, "港股ETF") for sublist in HK_ETF_MAP.values() for t in sublist]
                market_mode = "HK"
            elif st.session_state.active_file == 'US_ETFs.csv':
                try:
                    ...
                    market_mode = "US"
                except: st.error("⚠️ 找不到 US_ETFs.csv，請確保已上傳至 GitHub。")
        elif st.session_state.target == 'US_CSV' and st.session_state.active_file:
            try:
                df_csv = pd.read_csv(st.session_state.active_file)
                col = [c for c in df_csv.columns if c.lower() in ['ticker', 'symbol', '代號', 'sym']][0]
                selected_tickers = [(t, "美股") for t in df_csv[col].dropna().tolist()]
                market_mode = "US"
            except Exception as e: 
                st.error(f"⚠️ 讀取 {st.session_state.active_file} 失敗！原因: {e}")
        
        if selected_tickers:
            st.info(f"🚀 龍魂發動！正在執行絕對死刑過濾 (火力全開，掃描 {len(selected_tickers)} 隻標的)...")
            results = []; stop_loss_triggered = []; pb = st.progress(0)
            
            # 解除封印：掃描全陣列
            scan_list = selected_tickers
            ...
            if stop_loss_triggered:
                ...
                stop_loss_container.success("🛡️ 戰場安全，名單內暫無跌穿 10-EMA 之標的。")
            ...
            if results:
                ...
                for r in results:
                    ...
                    """, unsafe_allow_html=True)
            else: st.warning("💤 經過嚴格死刑過濾後，萬人坑內無生還者 (連 0981 都已被絕殺)。")
            ...
            with st.spinner("正在繪製全黑戰術圖表..."):
                try:
                    ...
                    if not df_chart.empty:
                        ...
                        fig.add_trace(go.Scatter(x=dates_chart, y=df_chart['EMA10'], mode='lines', name='10 EMA 止損', line=dict(color='orange', width=2, dash='dot')), row=1, col=1)
                        ...
                        fig.update_layout(template="plotly_dark", paper_bgcolor='#0e1117', plot_bgcolor='#111111', height=950, barmode='overlay',
