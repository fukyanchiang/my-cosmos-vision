...
# 第一行 100% 顯示羅輯
st.info("🐲 羅輯：11項死刑 Foul 制 (直接出局) ➡️ 7大指標海選 (RS>60, EJ>85, SE>75, OBV 1/7, NetFlow>0, 兵力勝, 集中度<70%) ➡️ 權重評分排序")
...
def plot_heavy_chart(ticker, df):
    ...
    fig.add_trace(go.Scatter(x=dates, y=df['Close'].ewm(span=10).mean(), line=dict(color='orange', width=2), name='10 EMA'))
...
if st.button("📡 啟動雷達掃描 (一擊必殺)"):
    ...
    for i, t in enumerate(all_t):
        pb.progress((i+1)/len(all_t)); status.text(f"掃描標的: {t}")
        df = smart_fetch(t)
    ...
    if sl_list:
        st.error(f"🚨 止損預警 (現價跌穿 10EMA)：{', '.join(sl_list[:25])}")
    ...
    if passed:
        ...
        for s in passed:
            ...
            <div class="result-card">
                ...
                <div class="stats-text">RS:{s['details']['RS']} | EJ:{s['details']['EJ']} | SE:{s['details']['SE']} | Bias:{s['details']['Bias']}% | 🛡️ 止損防線:{s['details']['StopLoss']}</div>
            </div>
            """, unsafe_allow_html=True)
            st.plotly_chart(plot_heavy_chart(s['t'], s['df']), use_container_width=True)
    else: st.warning("目前市場無符合標的。")
