...
if st.session_state.tactic is None:
    st.markdown("<h1 style='text-align:center; color:#FFD700;'>🦅 爺孫必勝戰術指揮部</h1>", unsafe_allow_html=True)
...
# ================= 3. 第二步：操作台與羅輯顯示 =================
st.markdown(f"## 🛠️ 戰術：{st.session_state.tactic}")
# 100% 顯示羅輯
st.info("🐲 羅輯：11項死刑 Foul 制 ➡️ RS>60, EJ>85, SE>75, OBV 1/7, NetFlow>0 ➡️ 權重評分排序")

if st.button("⬅️ 返回首頁"): st.session_state.tactic = None; st.rerun()

c1, c2, c3 = st.columns([1, 1, 1.5])
with c1: asset = st.radio("資產類別", ["🏢 個股", "🧺 ETF"], horizontal=True)
with c2: mkt = st.radio("市場", ["🇺🇸 美股", "🇭🇰 港股"], horizontal=True)
with c3: scan_range = st.selectbox("選擇範圍", ["🌐 啟動全星系大規模搜索", "🎯 監控清單"])

# ================= 4. 舊系統重裝圖表 (188.0 精華) =================
def plot_v188_heavy_chart(ticker, df):
    ...
    fig.update_layout(height=900, template='plotly_dark', paper_bgcolor='black', plot_bgcolor='black', xaxis_rangeslider_visible=False, showlegend=False)
    return fig
...
if st.button("📡 啟動雷達掃描"):
    ...
    for i, t in enumerate(all_t):
        pb.progress((i+1)/len(all_t)); status.text(f"分析中: {t}")
        df = smart_fetch(t)
    ...
    if sl_list:
        st.error(f"🚨 止損預警 (跌穿 10EMA)：{', '.join(sl_list[:20])}")
    ...
    if passed:
        ...
        for s in passed:
            ...
            <div class="result-card">
                <span style="float:right; border:1px solid #FFF; padding:2px 8px;">{s['sector']}</span>
                <div style="font-size:32px; font-weight:bold; color:#00FFFF;">{s['t']} {s['stage']}</div>
                <div style="font-size:30px; margin:10px 0;">{s['icons']}</div>
                <div class="stats-text">RS:{s['details']['RS']} | EJ:{s['details']['EJ']} | SE:{s['details']['SE']} | Bias:{s['details']['Bias']}% | 🛡️ 止損:{s['details']['StopLoss']}</div>
            </div>
            """, unsafe_allow_html=True)
            st.plotly_chart(plot_v188_heavy_chart(s['t'], s['df']), use_container_width=True)
