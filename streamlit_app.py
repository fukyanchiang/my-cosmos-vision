...
if st.session_state.page == 'HOME':
        ...
        if st.session_state.target == 'HK_STOCK':
            ...
            if st.session_state.active_file == 'HK_ETF':
                ...
                try:
                    ...
                    market_mode = "US"
                except: st.error("⚠️ 找不到 US_ETFs.csv。")
            ...
            try:
                ...
                market_mode = "US"
            except: st.error(f"⚠️ 讀取 {st.session_state.active_file} 失敗，請確定有 Upload CSV。")
        ...
        if selected_tickers:
            ...
            if results:
                ...
                for r in results:
                    ...
                    """, unsafe_allow_html=True)
            else: st.warning("💤 經過嚴格死刑過濾後，萬人坑內無生還者 (0981 絕對死刑)。")
            ...
            with st.spinner("正在繪製全黑戰術圖表..."):
                try:
                    ...
                    if not df_chart.empty:
                        ...
                        fig.add_trace(go.Bar(y=(bins[:-1]+bins[1:])/2, x=counts, orientation='h', marker_color='rgba(136,136,136,0.4)', name='重貨區 HVN', hoverinfo='skip', xaxis='x6', yaxis='y1'))
    ...
    st.markdown(f"<h1 style='text-align:center;'>施工中</h1>", unsafe_allow_html=True)
