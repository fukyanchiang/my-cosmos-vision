...
HK_ETF_MAP = {
    ...
    "H3. 行業板塊": "3134.HK 2845.HK 9845.HK 3136.HK 3069.HK 3174.HK 2820.HK 3133.HK 3111.HK 3141.HK 3148.HK 3149.HK 2842.HK 3120.HK 2806.HK 3143.HK 3137.HK 3051.HK".split(),
...
if st.session_state.page == 'HOME':
    ...
    # 🔥 ATH 改為 Checkbox，解決撞掣問題
    c_ath, c_btn = st.columns([3, 1])
    with c_ath:
        is_ath_mode = st.checkbox("🔥 啟動 ATH 歷史新高極致過濾 (只選 52周高位 93% 以上標的)")
    with c_btn:
            ...
            with st.spinner("正在繪製全黑戰術圖表..."):
                try:
                    ...
                    if not df_c.empty:
                        ...
                        fig.add_trace(go.Scatter(x=star_dates, y=stars['Volume'], mode='markers', marker=dict(symbol='star', size=10, color='gold'), name='大戶星星'), row=2, col=1)

                        # 能量副圖
                        add_energy_subplots(fig, df_c, dates_chart, row_start=3)
                        
                        # 鎖死黑底
                        fig.update_layout(template="plotly_dark", paper_bgcolor='#0e1117', plot_bgcolor='#111111', height=950, barmode='overlay', xaxis6=dict(overlaying='x1', anchor='y1', side='top', showticklabels=False, range=[0, max_c*4]), xaxis=dict(type='category', showticklabels=False), showlegend=False)
