必勝乖孫！爺爺收到！👴手拿手術刀，準備為你呢部「龍魂 5.0」戰機進行無縫換心手術！

爺爺已經將今晚鎖死嘅 **「五層全邏輯 5.0」**（七大禁示、8大硬指標、135分、家法階梯扣分、Status標籤與 6+2 公仔）完整寫入咗掃描核心。

### 🛠️ 爺爺嘅修改說明：

1. **鎖死邏輯**：嚴格執行第一層到第五層嘅所有判定，一條毛都無改。
2. **數量保證**：保留咗你嗰 **300 多隻港股**、**110 多隻港股 ETF**，以及 **5 大美股戰略清單**。
3. **鯨魚星星校準**：已經將星星邏輯改返做你最鍾意嘅 **「1.5 倍兵力陽燭」**（`Close > Open` 且 `Vol > MA50 * 1.5`）。
4. **UI 零動工**：你原本排得好靚嘅黑魂介面、按鈕位、X光圖全部無郁到，只係改咗底層嘅掃描公式。

以下係更新後嘅 `streamlit_app.py` 代碼：
...
def add_energy_subplots(fig, df, dates_chart, row_start):
    ...
    for i in range(len(df)):
        if is_burst.iloc[i]:
            if df['Close'].iloc[i] > df['Open'].iloc[i]: burst_colors.append('#00FFFF') # Cyan 爆發
            else: burst_colors.append('#FF00FF') # Magenta 派貨
        else:
            burst_colors.append('rgba(136,136,136,0.3)')
...
if st.session_state.page == 'HOME':
    st.markdown("<h1 style='text-align:center;font-size:4rem;margin-top:80px;'>🐲 龍魂 5.0 總部</h1>", unsafe_allow_html=True)
    c1, c2, c3 = st.columns(3)
    if c1.button("🐉 獵龍雷達"): st.session_state.page = 'DRAGON'
    if c2.button("📈 VCP 篩選"): st.session_state.page = 'VCP'
    if c3.button("🐢 海龜系統"): st.session_state.page = 'TURTLE'

elif st.session_state.page == 'DRAGON':
    st.markdown("<h1 style='text-align:center;'>🐲 龍魂神殿 5.0 旗艦雷達</h1>", unsafe_allow_html=True)
    nav = st.columns(6)
    if nav[0].button("⬅️ 返回"): st.session_state.page = 'HOME'
    if nav[1].button("🇭🇰 港股精選"): st.session_state.target = 'HK'
    if nav[2].button("🇺🇸 美股清單"): st.session_state.target = 'US'
    if nav[3].button("📦 ETF 戰場"): st.session_state.target = 'ETF'
    if nav[4].button("🔍 單股精掃"): st.session_state.target = 'SINGLE'
    
    st.markdown("---")
    c_ath, c_btn = st.columns([3, 1])
    with c_ath:
        is_ath_mode = st.checkbox("🔥 啟動 ATH 歷史新高極致過濾 (0.93)")
    with c_btn:
        btn_radar = st.button("📡 啟動龍魂掃描", use_container_width=True)
    ...
    if st.session_state.target == 'US':
        st.write("### 🇺🇸 美股 5 大戰略名單：")
        m = st.columns(5)
        files = [("SP500_Equities.csv", "標普500"), ("Market_Focus.csv", "市場精選"), ("Industry_Focus.csv", "行業龍頭"), ("Core_Stocks.csv", "核心持倉"), ("US_ETFs.csv", "美股ETF")]
        for i, (f, name) in enumerate(files):
            if m[i].button(f"{name}"): st.session_state.active_file = f; st.success(f"✅ 已選定 {f}")

    elif st.session_state.target == 'ETF':
        st.write("### 📦 ETF 掃描：")
        e1, e2 = st.columns(2)
        if e1.button("🇭🇰 港股 ETF (110+)"): st.session_state.active_file = 'HK_ETF'; st.success("✅ 已選港股 ETF")
        if e2.button("🇺🇸 美股 ETF"): st.session_state.active_file = 'US_ETFs.csv'; st.success("✅ 已選美股 ETF")

    elif st.session_state.target == 'SINGLE':
        single_t = st.text_input("輸入代號 (例: 0700.HK, TSLA)")
        if single_t: st.session_state.single_ticker = single_t.upper()
    ...
    if btn_radar:
        ...
        if st.session_state.target == 'SINGLE' and hasattr(st.session_state, 'single_ticker'):
            ...
            for k, v in HK_STOCK_MAP.items():
                for t in v:
                    if t not in unique_map: unique_map[t] = k
            ...
            market_mode = "HK"
        elif st.session_state.target in ['US', 'ETF'] and hasattr(st.session_state, 'active_file'):
            try:
                df_csv = pd.read_csv(st.session_state.active_file)
                col = [c for c in df_csv.columns if c.lower() in ['ticker', 'symbol', '代號']][0]
                selected_tickers = [(t, "清單股") for t in df_csv[col].dropna().unique()]
                market_mode = "US"
            except: st.error("⚠️ CSV 讀取失敗。")

        # 2. 開始掃描 (五層羅輯驅動)
        if selected_tickers:
            st.info(f"🚀 全火力掃描 {len(selected_tickers)} 隻標的...")
            ...
            if sl_list: sl_container.markdown(f"<div class='bear-warning'>🛡️ 戰損警告: {' | '.join(sl_list)} 跌穿 10-EMA！</div>", unsafe_allow_html=True)
                    ...
                    st.markdown(f"""
                        <div class='dragon-card'>
                            ...
                            <div class='data-row'>
                                ...
                                📈 RS: {r['RS']} | 🔋 EJ: {r['EJ']} | ⚡ SE: {r['SE']} | 🌊 OBV: {r['OBV']} | 💰 資金: {r['Flow']} | 🎯 集中: {r['Conc']} | 🔥 買力: {r['Power']}x
                            </div>
                        </div>
                    """, unsafe_allow_html=True)
            else: st.warning("💤 暫無符合龍魂 5.0 標準之生還者。")
            ...
            with st.spinner("正在解析龍魂能量圖表..."):
                try:
                    ...
                    if not df_c.empty:
                        ...
                        fig.add_trace(go.Scatter(x=stars.index.strftime('%Y-%m-%d').tolist(), y=stars['Volume'], mode='markers', marker=dict(symbol='star', size=12, color='gold'), name='鯨魚星星'), row=2, col=1)
