...
def load_csv_lists():
    ...
    # 建立舊名單集合防雙撞
    existing_us_eq = {sym for lst in US_STOCK_MAP.values() for sym in lst}
    existing_us_etf = {sym for lst in US_ETF_MAP.values() for sym in lst}
    
    for file in csv_files:
        try:
            # 判斷是否為 ETF (根據你提供的 "13. US MARKET FOCUS ETFS" 邏輯)
            is_etf = "13. US MARKET FOCUS ETFS" in file or "ETF" in file.upper()
            
            # 提取 List 名字
            if " - " in file: cat_name = file.split(" - ")[-1].replace(".csv", "").strip()
            else: cat_name = file.replace(".csv", "").strip()
            
            # 安全讀取 CSV
            try: df = pd.read_csv(file, on_bad_lines='skip', engine='python')
            except: continue
            
            # 如果無 Header，暴力搜尋 SYMBOL
            if 'SYMBOL' not in df.columns:
                with open(file, 'r', encoding='utf-8', errors='ignore') as f:
                    lines = f.readlines()
                header_idx = next((i for i, l in enumerate(lines) if 'SYMBOL' in l), -1)
                if header_idx != -1:
                    df = pd.read_csv(file, skiprows=header_idx, on_bad_lines='skip', engine='python')
                    
            if 'SYMBOL' in df.columns:
                symbols = df['SYMBOL'].dropna().astype(str).str.strip().tolist()
                clean_sym = [s for s in symbols if s and " " not in s and len(s) < 10]
                
                target_dict = US_ETF_MAP if is_etf else US_STOCK_MAP
                target_set = existing_us_etf if is_etf else existing_us_eq
                
                added = []
                for s in clean_sym:
                    if s not in target_set:  # 防雙撞
                        target_set.add(s)
                        added.append(s)
                        
                if added:
                    new_cat = f"🌟 CSV: {cat_name}"
                    if new_cat not in target_dict: target_dict[new_cat] = []
                    target_dict[new_cat].extend(added)
        except: continue
...
def analyze_dragon_soul(ticker, df, market_type="HK"):
    ...
    # 🛑 7 項鐵律死刑
    if v.iloc[-1] > ma20_v*1.5 and c.iloc[-1] < o.iloc[-1]*0.97: return False, 0, "", "", {}
    ...
    if v_60.iloc[max_v_idx] > ma20_v*4 and c.tail(60).iloc[max_v_idx] < o.tail(60).iloc[max_v_idx]: return False, 0, "", "", {}
    if v.iloc[-1] > ma20_v*5: return False, 0, "", "", {}
    if (h.iloc[-1] - max(o.iloc[-1], c.iloc[-1])) > abs(c.iloc[-1]-o.iloc[-1])*2: return False, 0, "", "", {}

    # 🐲 海選基礎門檻
    if not (rs > 55 and ej > 75 and se > 65 and net_flow_20 > 0): return False, 0, "", "", {}

    # 🏆 分數與 4 項陰險扣分
    score = (rs * 0.4) + (ej * 0.3) + (se * 0.3)
    penalty_score = 0; penalties = []
