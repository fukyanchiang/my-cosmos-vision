...
def check_stop_loss(df):
    """偵測最近 2 個交易日係咪跌穿 10-day EMA"""
    if len(df) < 15: return False
    ema10 = df['Close'].ewm(span=10, adjust=False).mean()
    # 條件：今日收市跌穿 EMA10，且尋日或前日仲喺 EMA10 之上
...
def scan_dragon_logic(df, ticker, sector_name, market="HK"):
    ...
    # --- 第一層：7 大禁示 ---
    if v.iloc[-1] > ma20_v.iloc[-1]*1.5 and pct.iloc[-1] < -0.03: return None
    if pct.iloc[-1] > 0 and netvol.iloc[-1] < 0: return None
    if v.iloc[-1] > ma20_v.iloc[-1]*3 and pct.iloc[-1] < 0.02: return None
    if pct.iloc[-2] > 0.05 and v.iloc[-1] < v.iloc[-2]*0.5: return None
    if bias > 15 and v.iloc[-1] >= v.tail(252).max() * 0.9: return None
    obv = (np.sign(pct) * v).cumsum()
    if pct.tail(5).sum() > -0.01 and obv.iloc[-1] < obv.iloc[-5]: return None
