...
def scan_dragon_logic(df, ticker, sector_name, market="HK", mode='NORMAL', force_return=False):
    ...
    # =======================================================
    # 🏆 第三及四層：權重計分與地獄扣分 (🤖 雙核心分流)
    # =======================================================
    score = 100.0; bonus_list = []
    core_p = 0; bias_p = 0; final_score = 0
    ...
    if mode == 'VCP':
        ...
        # 🚨 VCP嚴打犯規：有任何 7 大禁令扣分，直接重罰 100 分確保沉底！
        if foul_points > 0:
            final_score -= 100
            
    else:
        # 🛡️ 孫仔的原本完美邏輯 (NORMAL 模式)
        score = 100.0; bonus_list = []
